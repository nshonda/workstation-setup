...
- **Slack MCP** — two workspaces: `slack-onerhino` (personal/oneRhino) and `slack-basis` (work/Basis). Use the one matching the current workspace context.
  ...
  - **Slack message structure matters.** Use `\n` newlines to separate paragraphs, introduce lists, and break sections. A message without newlines renders as a wall of text. Never jam sentences together — if content has distinct sections (intro, list, notes, cc), separate them with blank lines (`\n\n`). URLs followed by a new sentence MUST have a newline or period+space between them.
  - **Only bullet list items, never prose.** Intro paragraphs and closing text must be plain text — NOT bullets. Only use `• ` for actual list items in the middle.
    - WRONG: `• Hey, here's an update:\n• item 1\n• item 2\n• Let me know if questions.` — renders the entire message as a bulleted list
    - RIGHT: `Hey, here's an update:\n\n• item 1\n• item 2\n\nLet me know if questions.` — intro and closing are plain paragraphs, only items are bulleted
