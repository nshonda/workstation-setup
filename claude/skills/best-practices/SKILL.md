...
Always use safe DOM APIs:
- Use `element.textContent` for plain text (safe by default)
- Never insert untrusted data via unsafe DOM properties
- If HTML rendering is needed, use a sanitizer library like DOMPurify
- Set cookies server-side with `Secure; HttpOnly; SameSite=Strict` flags
...
**Parser-blocking DOM write methods** — These block HTML parsing and are flagged by Lighthouse. Use `createElement` + `appendChild` instead.
...
// ❌ Application Cache (removed from browsers)
...
**Dynamic script loading (the correct way):**
```javascript
// ✅ Use createElement for dynamic scripts
